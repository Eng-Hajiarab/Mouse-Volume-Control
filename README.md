...
### نصب و اجرا
روش اول: ( بدون نیاز به نصب برنامه AutoHotkey )
1. **اجرای فایل `Mouse Volume Control.exe`** با دوبار کلیک
2. اسکریپت در **System Tray** فعال می‌شود و آماده استفاده است

> نکته: برای اجرای خودکار هنگام روشن شدن سیستم، فایل `Mouse Volume Control.exe` را در پوشه `Startup` ویندوز قرار دهید.
...
> نکته: برای اجرای خودکار هنگام روشن شدن سیستم، فایل `Mouse Volume Control.ahk` را در پوشه `Startup` ویندوز قرار دهید.
